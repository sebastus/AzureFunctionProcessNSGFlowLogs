...
{
    log.Info($"C# NSG Flow Logs Ingest Test (timer) function executed at: {DateTime.Now}");

    var attributes = new Attribute[]
    {
        new QueueAttribute("nsgprocessing"),
        new StorageAccountAttribute("AzureWebJobsStorage")
    };

    var queue = await inputQueue.BindAsync<CloudQueue>(attributes);

    var messages = await queue.GetMessagesAsync(1);
